...
\subsection{Problem 1, proposed by Prajit Adhikari(Nepal)}
...
\subsection{Problem 2, proposed by Shining Sun(USA)}
...
\begin{proof}
    We proceed by downward induction. Assume, for some $k \le 2026$, that $m+1 \mid T_k(m)$. Then,
    \[
    m+1 \mid T_k(m) = f(m+T_{k-1}(m)) \mid m+1+T_{k-1}(m)
    \]
    Therefore we get that $m+1 \mid T_k(m) \implies m+1 \mid T_{k-1}(m)$, so we reduce this to $m+1 \mid f(m)$. Combining
    with the first condition we get that $f(m) \mid m+1$ which gives $f(m) = m+1$ for $m>0$. However setting $m=1$ in the first
    condition gives us $f(0) = 1$, therefore $f(m) = m+1$ for all $m\ge0$.
\end{proof}

\newpage{}

\subsection{Problem 3, proposed by Lasitha Jayasinghe(Sri Lanka)}
...
\begin{proof}
    It suffices to show that quadrilaterals $BOHC$ and $APHO$ are cyclic, from which the radical axis theorem finishes.

    \begin{claim}
        Quadrilateral $BOHC$ is cyclic.
        \begin{proof}
            We have
            \[
    ...
    \begin{claim}
        Quadrilateral $APHO$ is cyclic.
        \begin{proof}
            We have that:
            \begin{align*}
                \angle HOA &= 360^\circ - \angle AOB - \angle BOH\\ 
                &= 180^\circ - 2\angle ACB + \angle HCB \\ 
                &= 270^\circ - 2\angle ACB - \angle CBA
            \end{align*}
            Also, we have
            \begin{align*}
                \angle APH
                &= 180^\circ - \angle MBA \\ 
                &= 180^\circ - (\angle MBC + \angle CBA) \\
                &= 180^\circ - (90^\circ - \frac{\angle BOC}{2} + \angle CBA) \\
                &= 180^\circ - (90^\circ - \angle BAC + \angle CBA) \\
                &= 150^\circ - \angle CBA
            \end{align*}
            Therefore we get that $\angle HOA + \angle APH = 420^\circ - 2(\angle ACB + \angle CBA) = 180^\circ$.
        \end{proof}
    \end{claim}
    Therefore radical axis on $(ABC)$, $APHO$, and $BOHC$ completes. However, it doesn't. We are left to prove that the three
    lines actually concur at some point, and are not parallel.
    \begin{claim}
       The lines $AP$, $OH$, and $BC$ are not parallel.
        \begin{proof}
            Assume not, for the sake of contradiction. Then, $BHOC$ becomes a cyclic trapezoid so $BH=OC$. Some simple length
            chasing gives
            \[
            BH = 2 CO \cos(\angle CBA)
            \]
            Which forces $\angle CBA = 60^\circ$, contradicting $\triangle ABC$ scalene. Therefore, the three lines are not
                parallel.
        \end{proof}
    \end{claim}
...
\subsection{Problem 4, proposed by Shining Sun(USA)}
...
\subsection{Problem 5, proposed by Basu Dev Karki(Nepal)}
...
\subsection{Problem 7, proposed by Prajit Adhikari(Nepal), Shreyash Sharma Bastola(Nepal)}
...
\subsection{Problem 8, proposed by Prajit Adhikari(Nepal), Kritesh Dhakal(Nepal)}
